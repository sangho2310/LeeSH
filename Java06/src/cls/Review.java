...
public class Review {
	public static void main(String[] args) {
		...
		MyModel mm2 = new MyModel(); // 객체가 다르면 형태와 메서드는 같지만 변수가 다름(다른공간)
//		String str = "안녕";
//		String str2= "반갑";
...
//MVC패턴 웹 유행 	(Model(구조체같은) View(Html화면) Controller(Java코드와 Html코드 연결))
// 사용할 변수를 지정해놓고 갖다 사용할 수 있는 클래스
class MyModel {
	// 변수들, 변수에 값을 대입하거나 사용할 수 있는 메서드만
	// 클래스 : 변수 + 함수를 뭉쳐놓은 것 (멤버변수 , 메서드 )
	...
	int getNum() { // 정수값 num을 사용한 곳에 return(전달)해줄 것이기 때문에 자료형은 int
		return num; // 이름이 중복되는 매개변수나 지역변수가 없으면 this.은 생력가능
	}
